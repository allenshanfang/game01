...
export function findWinner (board, y, x) {
  if (
    countTotal(board, y, x, 1, 0) + countTotal(board, y, x, -1, 0) >= 2 ||
    countTotal(board, y, x, 0, 1) + countTotal(board, y, x, 0, -1) >= 2 ||
    countTotal(board, y, x, 1, 1) + countTotal(board, y, x, -1, -1) >= 2 ||
    countTotal(board, y, x, 1, -1) + countTotal(board, y, x, -1, 1) >= 2
  ) {
    return board[y][x]
  }
